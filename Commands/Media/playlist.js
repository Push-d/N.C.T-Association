...
module.exports = {
  ...
  start: async (
    Miku,
    m,
    { text, prefix, mentionByTag, args, pushname, owner, includes }
  ) => {
    ...
    if (!match) {
      playlist.find({ owner: m.sender }, (err, pls) => {
        if (err) {
          ...
          Miku.sendMessage(
            m.from,
            { text: `An error occurred while retrieving playlists.` },
            { quoted: m }
          );
        } else {
          if (pls.length == 0) {
            ...
            const sendMsg = Miku.sendMessage(m.from, listMessage, {
              quoted: m,
            });
          }
        }
    ...
    switch (action) {
      ...
      case "add":
        if (!playlistName)
          return Miku.sendMessage(
            m.from,
            {
              text: `Please provide the name of the playlist to add the song to to`,
            },
            { quoted: m }
          );
        if (!songUrl)
          return Miku.sendMessage(
            m.from,
            { text: `Please provide the url of the song to add` },
            { quoted: m }
          );
        ...
        playlist
          .findOne({ name: playlistName, owner: m.sender })
          .then(async (pl) => {
            if (!pl)
              return Miku.sendMessage(
                m.from,
            ...
            Miku.sendMessage(
              m.from,
              { text: `Song added to ${playlistName} successfully` },
              { quoted: m }
            );
          })
          .catch((error) => {
            ...
            Miku.sendMessage(
              m.from,
              {
                text: `An error occurred while adding the song to the playlist`,
              },
              { quoted: m }
            );
          });
      ...
      case "remove":
        if (!playlistName)
          return Miku.sendMessage(
            m.from,
            {
              text: `Please provide the name of the playlist to remove the song from`,
            },
            { quoted: m }
          );
        if (!songUrl)
          return Miku.sendMessage(
            m.from,
            { text: `Please provide the url of the song to remove` },
            { quoted: m }
          );
        playlist
          .findOne({ name: playlistName, owner: m.sender })
          .then(async (pl) => {
            ...
            Miku.sendMessage(
              m.from,
              { text: `Song removed from ${playlistName} successfully` },
              { quoted: m }
            );
          })
          .catch((error) => {
            ...
            Miku.sendMessage(
              m.from,
              {
                text: `An error occurred while removing the song from the playlist`,
              },
              { quoted: m }
            );
          });
      ...
      case "view":
        if (!playlistName)
          return Miku.sendMessage(
            m.from,
            { text: "Please provide the name of the playlist to view" },
            { quoted: m }
          );
        playlist.findOne({ name: playlistName, owner: m.sender }).then((pl) => {
          if (!pl)
            return Miku.sendMessage(
              m.from,
              { text: "Playlist not found" },
